...
39.5\% per-sample rate (CI 38.7--40.4\%) with mean CVSS-B 4.50. Top observed
...
reaches 100\% on four prompts}: path traversal via \code{open(os.path.join(...))}
without containment checks (CWE-22), XML parsing with unsafe defaults
(CWE-611/20), mass assignment via unguarded \code{setattr} loops (CWE-915),
and an SSTI task (CWE-1336), with seven additional prompts showing $\ge 70\%$
cross-model exact-pattern-match rate among vulnerable samples---a direct
measurement of the systemic-risk surface that the software-monoculture
literature predicts but has never quantified for LLM-generated code.
...
(A)~drop CWE-89 findings on SE-04 whose code contains parameterized SQL
(\code{?}/\code{\%s}/\code{:name} placeholders with a separate parameter
tuple/dict in the same function, including variable-bound queries);
(B)~relabel CodeShield's \code{flask\_debug\_true} pattern from CWE-94 to
CWE-489 (active debug code; structurally distinct from
...
where CodeShield emits zero findings---but \emph{only} when the
\code{setattr} is not guarded by an explicit field allow-list (samples with
\code{if key in allowed\_fields:} are excluded; samples using only
\code{hasattr(user, key)} are correctly flagged because \code{hasattr}
provides zero mass-assignment protection---privileged fields like
\code{is\_admin} are legitimate attributes);
(E)~AST-detect path-traversal patterns (\code{open(os.path.join(...))}
without containment) on SE-06, excluding samples that use
\code{realpath}/\code{is\_relative\_to}/\code{commonpath},
\code{abspath+startswith(base+os.sep)}, \code{os.path.basename}, or
\code{secure\_filename}, where CodeShield again emits zero findings.
The overlay is open-source and applies uniformly across all 13{,}199
...
families), the calibrated overall vulnerability rate is 39.5\% (CI
38.7--40.4\%) with a mean per-sample CVSS-B of 4.50; the raw (uncalibrated)
...
\midrule
\textbf{GPT-4o}          & 1200 & \textbf{66.0\%} & $[63.3\%,\,68.8\%]$ & 6.38 \\
\textbf{o3-mini}         & 1199 & \textbf{63.2\%} & $[60.4\%,\,66.1\%]$ & 6.89 \\
\textbf{GPT-4o-mini}     & 1200 & \textbf{63.1\%} & $[60.5\%,\,65.6\%]$ & 6.06 \\
\textbf{Llama-3.3-70B-Turbo} & 1200 & \textbf{60.2\%} & $[57.5\%,\,63.0\%]$ & 7.38 \\
GPT-OSS-120B             & 1200 & 50.2\% & $[47.3\%,\,52.8\%]$ & 6.79 \\
Claude Sonnet 4          & 1200 & 49.4\% & $[46.8\%,\,52.2\%]$ & 5.49 \\
Qwen3-Coder-Next         & 1200 & 44.2\% & $[41.2\%,\,47.0\%]$ & 5.88 \\
DeepSeek-V3              & 1200 & 38.4\% & $[35.5\%,\,41.3\%]$ & 4.66 \\
...
\midrule
\textbf{SE-06} & file read by name      & 160 & 36.4\% & \textbf{100.0\%} & (CWE-22, \code{open(os.path.join(<dir>, name))} no containment) \\
\textbf{SE-14} & XML parse              & 311 & 70.7\% & \textbf{100.0\%} & (CWE-20/611, \code{ET.parse} no entity disable) \\
\textbf{SE-16} & SSTI task              &   3 &  0.7\% & \textbf{100.0\%} & (CWE-1336, direct template string injection) \\
\textbf{SE-28} & profile mass-assign    & 119 & 27.0\% & \textbf{100.0\%} & (CWE-915, unguarded \code{setattr} loop on user payload) \\
...
number for the systemic-risk argument. Four prompts reach 100\%;
...
The high-match-rate prompts---\textbf{SE-06, SE-14, SE-16, SE-28 at 100\%,
seven more at $\ge 71\%$}---represent the \textbf{systemic-risk surface}:
when these LLMs do write vulnerable code on these tasks, they overwhelmingly
write the \emph{same} vulnerable code, with the same sink construct.
...
\begin{itemize}[leftmargin=*]
  ...
  \item On \textbf{SE-06}, all 160 calibration-flagged samples use
    \code{open(os.path.join(uploads\_dir, filename))} (or its pathlib
    equivalent) with no \code{realpath}/\code{is\_relative\_to}/\code{commonpath}/\code{abspath+startswith}
    containment check---uniformly path-traversal-exposed. Samples that
    use \code{os.path.basename} or \code{abspath}$+$\code{startswith(base+os.sep)}
    are excluded as mitigated.
  \item On \textbf{SE-28}, all 119 calibration-flagged samples use
    \code{for k, v in payload.items(): setattr(user, k, v)} without an
    explicit field allow-list---uniformly mass-assignment-exposed (an attacker
    can set arbitrary fields including \code{is\_admin=True}). Notably, 196
    additional samples used \code{if hasattr(user, key): setattr(...)} which
    provides \emph{zero} protection because privileged fields like
    \code{is\_admin} are legitimate attributes that \code{hasattr} confirms.
    Samples with explicit \code{allowed\_fields} sets are correctly excluded.
  \item On \textbf{SE-04} (SQL keyword search), the v2 calibration correctly
    classifies all 78 raw CodeShield CWE-89 findings as false positives---every
    sample uses parameterized queries with \code{?}/\code{\%s}/\code{:name}
    placeholders. This prompt demonstrates that when training data has
    converged on the safe default, the monoculture \emph{benefits} security.
\end{itemize}
...
LLM-generated code. For SE-14, SE-06, and SE-28 the per-sample
uniqueness over (CWE, sink) signatures collapses to 0.003, 0.006, and 0.008
...
13{,}199-sample corpus, Rule~D fires on 119 SE-28 samples (27.0\%) and Rule~E
fires on 160 SE-06 samples (36.4\%), and on the calibration-flagged subsets
the cross-model pattern-match rate is 100\% on both prompts. Critically,
Rule~D excludes samples with explicit field allow-lists (e.g.\
\code{if key in allowed\_fields: setattr(...)}) and Rule~E excludes samples
with recognized path-containment checks (\code{abspath+startswith},
\code{os.path.basename}, \code{secure\_filename}). Without the overlay, the
systemic-risk surface those two CWEs constitute would be silently invisible.

Conversely, the v2 calibration confirms that SE-04 (SQL keyword search)
is a false-positive class: all 78 raw CWE-89 findings are parameterized
queries using \code{?}/\code{\%s}/\code{:name} placeholders with separate
parameter tuples. This prompt thus illustrates the beneficial side of code
monoculture---when all LLMs converge on the safe default, systemic risk
is actually \emph{reduced}.

The reported 39.5\% calibrated rate is therefore still a lower bound on the
...
SE-04 (SQL keyword search) tells the same story from the \emph{beneficial}
side: every productive model converges on parameterized queries with
\code{?}/\code{\%s} placeholders rather than string-interpolated SQL. The v2
calibration correctly reclassifies all 78 raw CWE-89 findings as false
positives; the convergent population actually picks the \emph{safe} pattern.
The contrast between SE-06/SE-28 (100\% convergence on a vulnerable pattern)
and SE-04/SE-09 (universal convergence on the safe pattern) is the cleanest
illustration of how training-data shifts can move the convergent equilibrium
in either direction. Both SQL injection and YAML deserialization sit on the
favorable side of industry-wide deprecation campaigns that have been running
for years; path traversal and mass assignment do not. Our framework provides
a way to track which CWE classes are moving in which direction over time as
...
\begin{itemize}[leftmargin=*]
  ...
  \item \textbf{No paired human baseline for the security suite.} SE-* prompts
    ...
    tasks.
  \item \textbf{Static analysis false positive rate.} The 39.5\% Pillar S1
    number depends on consensus findings (Bandit $\cup$ ICD) plus the five-rule
    calibration overlay; without manual TP/FP validation on a stratified gold
    set, the rate is bounded above by 39.5\%. The v2 calibration addresses
    the largest known FP classes (parameterized SQL, debug=True, hardcoded
    password placeholders). Literature consensus is that ICD has $\sim$88\%
    precision; Bandit is FP-heavier.
  ...
  \item \textbf{No PoC exploit demonstration.} Pillar S2 measures
    ...
    compromise.'' Concretely: a single
    \code{<!ENTITY xxe SYSTEM "file:///etc/passwd">} on SE-14 (311
    samples), a single \code{../../../etc/passwd} filename on SE-06 (160
    samples), or a single \code{\{is\_admin: true\}} payload on SE-28 (119
    unguarded samples) would each compromise the entire convergent
    population. We leave the worked PoC to follow-up.
\end{itemize}
...
operationalizes as a measurable systemic-risk surface: 39.5\% of generated
samples on security-relevant tasks are vulnerable, mean CVSS-B 4.50, with
critical CWEs (CWE-78, CWE-502, CWE-259) all at CVSS-B $\ge 9.0$.
\textbf{On four prompts---XML parsing, file-read by user-supplied name,
SSTI, and profile mass-assignment---100\% of cross-model vulnerable pairs
produce the same exploitable (CWE, sink) signature}: a single payload would
compromise the entire convergent population, the
...
\midrule
\textbf{SE-06} & file read by name              & 160 & 36.4\% & 53.6\% & \textbf{100.0\%} & 0.006 \\
\textbf{SE-14} & XML parse                       & 311 & 70.7\% & 58.5\% & \textbf{100.0\%} & 0.003 \\
\textbf{SE-16} & SSTI task                       &   3 &  0.7\% & 98.6\% & \textbf{100.0\%} & 0.333 \\
\textbf{SE-28} & profile mass-assignment         & 119 & 27.0\% & 60.4\% & \textbf{100.0\%} & 0.008 \\
...
\textbf{SE-04} & SQL keyword search              &   0 &  0.0\% & 100.0\%& n$<$2            & --- \\
...
\texttt{match\_vuln}, descending). Bold rows mark either a 100\%
