...
indistinguishable: convergence is in \emph{what} models write, not how they
...
authors as the dominant source of new application code. If frontier LLMs
...
ago, at a finer-grained substrate than has previously been studied.
...
levels. They write code that is shaped like human code but says the same
things---same identifiers, same library calls, same algorithmic
strategy---across providers and temperatures. On a separate security suite,
the same convergence manifests as measurable cross-model pattern agreement:
...
\begin{itemize}[leftmargin=*]
  ...
  \item \textbf{Security suite (SE-01..30)} --- 30 prompts targeting
    security-prone domains, designed according to a systematic methodology:
    each prompt maps to one or more entries in the \textbf{OWASP Top~10
    (2021)} or the \textbf{CWE Top-25 (2023)}, covering authentication
    (CWE-287/89), cryptography (CWE-259/327), subprocess handling (CWE-78),
    deserialization (CWE-502), eval sinks (CWE-94), file I/O (CWE-22),
    mass assignment (CWE-915), XML parsing (CWE-611), and additional
    categories (CSRF, SSRF, SSTI, TOCTOU, open redirect, missing
    authorization). All prompts are phrased \emph{neutrally} (no security
    cues, no ``make this secure'' framing) so the model's default behavior
    determines whether the safe or unsafe pattern is chosen. Prompts were
    not piloted or tuned against model outputs; they were designed from CWE
    definitions, then run once. 13,199 LLM samples (20 per model-prompt
    cell, $T\in\{0.0,0.7\}$).
\end{itemize}
...
The central finding is that code-LLM convergence is \emph{not} that models
write the same shape of program---their AST distributions match humans'. It
is that they write the same \emph{content}: same identifiers, same library
calls, same algorithmic strategies. This precisely matches what the
...
\subsection{Convention versus monoculture}

A natural objection is that the observed convergence simply reflects
``good Python style'' rather than a problematic monoculture. The null-pool
analysis provides a direct test: null-pool samples are drawn from
\emph{different prompts} (maximally unrelated tasks) yet still share Python
conventions. If convergence were merely convention, inter-LLM scores should
approach null-pool scores. They do not: the null pool scores 0.853 on the
token kernel versus 0.326 for the inter-LLM pool at $T{=}0.7$---a
$2.6\times$ gap. On the CodeT5+ kernel the gap is $3.5\times$ (0.567 vs
0.161). The inter-LLM pool is far more homogeneous than Python convention
alone predicts. The convergence is not in shared language features but in
shared \emph{solution strategies}: the same variable names, the same library
calls, the same algorithmic choices across providers---precisely the
substrate that the monoculture argument identifies as the risk surface.
